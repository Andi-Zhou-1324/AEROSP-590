function [F] = roe_1D_EntropyFix(UL, UR)
    ...
    u_star_R   = (rho_R.*u_R + alpha(3).*(u - a))./(rho_L + alpha(3));
    ...
    if (lambda_3_L < 0)&&(lambda_3_R > 0)
        lambda(3) = lambda_3_R.*((lambda(3)-lambda_3_L)./(lambda_3_R - lambda_3_L));
        F = F_R + lambda_3.*alpha(3)*K(:,3);
        F = F';
    end
